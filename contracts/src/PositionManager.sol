...
contract PositionManager is ReentrancyGuard, Ownable {
        ...
        onlyOpenPosition(positionId)
    {
        ...
        uint256 requiredMargin = (position.size * position.entryPrice) / position.leverage;
        require(remainingMargin >= requiredMargin, "Removal would trigger liquidation");
     ...
     */
    function updateUnrealizedPnl(uint256 positionId, uint256 currentPrice) 
        external 
        onlyOpenPosition(positionId)
    {
        Position storage position = positions[positionId];
    ...
    function liquidatePosition(uint256 positionId, uint256 liquidationPrice) 
        ...
        onlyOpenPosition(positionId)
    {
        // TODO: Add access control for liquidation engine
        Position storage position = positions[positionId];
        ...
        emit PositionLiquidated(positionId, position.trader, liquidationPrice, loss);
    }
     ...
     */
    function calculateHealthFactor(uint256 positionId, uint256 currentPrice) 
        external 
        view 
        returns (uint256 healthFactor) 
    {
        ...
        uint256 currentValue = (position.size * currentPrice) / position.entryPrice;
        uint256 requiredMargin = (position.size * position.entryPrice) / position.leverage;
